...
# Assuming these exist in your core.obb module based on previous context
# If not, they must be implemented or imported from the correct location
try:
    ...
    # Fallback stubs for development if dependencies aren't fully linked yet
    # In a real build, these imports must resolve to your actual C-extension or python modules
    print("WARNING: Direct OBB editing dependencies (nba2k20_obb/obb_encrypt) not found. Editing disabled.")
    Obb = None
    TYPE_COMPRESSED = b'\x01'
    TYPE_ZLIB = b'\x02'
    TYPE_NAMES = {b'\x01': 'COMPRESSED', b'\x02': 'ZLIB'}
    BuiltEntry = None
    def parse_entry(*args): return None, [], 0, b''
    def apply_entries(*args): pass
    def sha256_path(*args): return ""
    def update_archive_crc32(*args): return 0
    def verify_output(*args): pass
...
class DirectObbSession:
    ...
    def __init__(self, path: Path, cache_dir: Optional[Path] = None):
        if Obb is None:
            raise RuntimeError("OBB editing engine not available. Missing dependencies.")
        ...
        if cache_dir is None:
            cache_dir = self.path.parent / f".{self.path.stem}_cache"
    ...
    def entry_type(self, index: int) -> tuple:
        entry = self._entry(index)
    ...
    def stage_iff(
        ...
        staged = StagedEntry(
            ...
            decompressed_sha256=hashlib.sha256(decompressed_data).hexdigest().upper(),
        )
